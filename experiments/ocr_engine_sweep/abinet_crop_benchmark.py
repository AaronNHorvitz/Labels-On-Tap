#!/usr/bin/env python
"""Benchmark MMOCR ABINet recognition over detected OCR crops.

ABINet is a scene-text recognizer, not a complete detector-plus-recognizer OCR
pipeline by itself. This experiment therefore reuses boxes from an existing
detector/OCR run, crops those regions, runs MMOCR's ABINet recognizer on each
crop, and writes the same normalized OCR JSON/CSV artifacts used by the
field-support scorer.

The resulting latency is crop preparation plus ABINet recognition latency. It
...
class ABINetCropRow:
    """One ABINet crop-recognition benchmark row."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--model", default="ABINet")
    parser.add_argument("--max-crops-per-image", type=int, default=96)
    parser.add_argument("--padding", type=int, default=2)
    parser.add_argument("--run-name", default="abinet-openocr-crops-smoke-30")
...
class ABINetRecognizer:
    """Lazy wrapper around MMOCR's ABINet text-recognition inferencer."""

    def __init__(self, *, model: str, device: str) -> None:
        """Load MMOCR ABINet on the requested device."""

        try:
            from mmocr.apis import TextRecInferencer
        except Exception as exc:  # pragma: no cover - optional experiment deps
            raise RuntimeError(f"MMOCR ABINet dependencies are unavailable: {exc}") from exc
    ...
    def recognize(self, crop_paths: list[Path], batch_size: int) -> list[tuple[str, float]]:
        """Recognize text from crop file paths with ABINet."""
...
def write_ocr_json(output_dir: Path, image_path: Path, result: OCRResult) -> Path:
    ...
    ocr_dir = output_dir / "ocr" / "abinet_openocr_crops"
...
def summarize(rows: list[ABINetCropRow]) -> dict:
    """Summarize ABINet crop benchmark rows."""

    ok_rows = [row for row in rows if row.status == "ok"]
    latencies = [row.total_ms for row in ok_rows]
    return {
        "abinet_openocr_crops": {
...
def main() -> None:
    """Run ABINet recognition over detected crop boxes."""
    ...
    recognizer = ABINetRecognizer(model=args.model, device=args.device)
    rows: list[ABINetCropRow] = []
    ...
    for index, row in enumerate(box_rows, start=1):
        ...
        print(f"[abinet_openocr_crops] {index}/{len(box_rows)} {relative_path(image_path)}")
        ...
        try:
            ...
            crop_boxes = []
            with tempfile.TemporaryDirectory(prefix="abinet-crops-") as temp_dir_name:
            ...
            result = OCRResult(
                ...
                blocks=blocks,
                source="local MMOCR ABINet over OpenOCR crops",
                ocr_ms=total_ms,
                total_ms=total_ms,
            )
    ...
    with (output_dir / "rows.csv").open("w", newline="", encoding="utf-8") as file:
        fieldnames = list(asdict(rows[0]).keys()) if rows else list(ABINetCropRow.__dataclass_fields__)
    ...
    print()
    print(f"Wrote ABINet crop benchmark outputs to {output_dir}")

...
The corrected audit workflow is now generated by `experiments/typography_preflight/build_audit_dataset.py` under gitignored `data/work/typography-preflight/audit-v4/`. It separates provenance labels from model-facing decision labels:
...
`audit-v4` also moves leading/trailing whitespace and crop-boundary artifacts out of the visible `incorrect` header class and into the review path. Those artifacts are useful for deterministic string/crop-boundary tests, but they can be visually indistinguishable from a correct heading and should not train an image classifier as clean "incorrect" examples.
...
**Next comparison:** After `audit-v4` passes inspection, train side-by-side multiclass models for both typography decisions:

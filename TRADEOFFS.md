...
**BERT bridge correction:** DistilRoBERTa is no longer only a documented future
idea. The app now includes an optional runtime field-support bridge. OCR still
reads the image first; DistilRoBERTa scores candidate OCR text against the
application field value. The bridge can supply evidence for text-field matches,
but source-backed rules and reviewer queues still make the final disposition.
The saved model artifact is `327 MB` and remains outside Git under
`data/work/field-support-models/distilroberta-field-support-v1-runtime/model/`;
deployment must mount or copy it separately.
...
**Valid model-selection comparison:** The latest typography model-selection
run uses one statistically controlled dataset and protocol: `audit-v6`, with
`6,000` train / `1,500` validation / `1,500` untouched test crops. Every base
learner used the same target (`boldness_label`) and split. Ensemble stackers
trained on five-fold out-of-fold probabilities from all base learners,
including the MobileNetV3 CNN. Reject thresholds were tuned on validation only.
...
Result: the CNN is the safest individual base learner. The logistic stacker is
the raw-F1 winner, but its false-clear rate is too high for automatic
government-warning clearance. The LightGBM/XGBoost reject ensembles are the

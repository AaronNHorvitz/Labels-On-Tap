...
## Current Model-Selection Checkpoint

The latest typography model-selection run is the statistically valid comparison
to cite. It uses the same `audit-v6` dataset for every model, with `6,000`
train / `1,500` validation / `1,500` untouched test crops. Base learners use
five-fold out-of-fold train predictions. Ensemble stackers are trained on those
OOF probability vectors from every base learner, including the MobileNetV3 CNN.
Validation is used only for reject-threshold tuning; final metrics are test
only.
...
Decision: keep the MVP runtime on the real-adapted JSON logistic typography
